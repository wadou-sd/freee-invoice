...
- 名寄せ6組を統合、平沼・西川・県立2校は分離。対象開始=2025-08-01（当期。前期は対象外）
...
✅ 2026-07-06 当期の請求状況チェック（partner_id基準に修正）
- 当初の名前/ID混在バグを修正。当期(2025-08〜)の未請求は 上林・大井常明・福島陽子 の3件のみ（他は請求済み）
- 対象開始を 2025-08-01（当期）に確定

🔄 2026-07-06 Phase 5 着手: プロジェクト骨組み＋トークン自動更新
- 依存ゼロ(標準ライブラリ)方針。src/config.py, src/store.py(Supabase REST), src/freee_token.py, scripts/check_token.py を追加
- .env.example / .gitignore 追加
- 締め=月末、完了通知=ログ で合意
- 次: VPSデプロイ（git pull＋.env作成）と Supabase service_role キーの設定 → check_token.py で疎通確認
